// lib/animated_ai_interface.dart
...
/// Widget que transiciona entre esfera (usuário) e faixa de onda (IA)
class AnimatedAIInterface extends StatefulWidget {
  // A semântica desses parâmetros mudou:
  // isUserSpeaking agora significa "mostrar a faixa de onda (porque a IA está falando)"
  // isAISpeaking agora significa "mostrar a esfera (porque o usuário está ativo/falando)"
  final bool isUserSpeaking; // Se true, mostra a faixa de onda (IA falando)
  final bool isAISpeaking;   // Se true, mostra a esfera (usuário ativo)
  final double soundLevel;
  final String transcribedText; // Texto transcrito da IA
  final VoidCallback onTap;
  final Widget orbWidget; // Widget da esfera (representando o usuário)
...
class _AnimatedAIInterfaceState extends State<AnimatedAIInterface>
  ...
  // Controle de estado
  bool _showingWave = false; // Agora significa "mostrando a faixa de onda (IA)"
  String _displayText = '';
  ...
  void didUpdateWidget(AnimatedAIInterface oldWidget) {
    ...
    // Lógica clara e consistente de transição com a nova semântica
    // Se widget.isUserSpeaking for true, queremos mostrar a faixa de onda (IA está falando)
    ...
    if (shouldShowWave != wasShowingWave) {
      ...
      if (shouldShowWave) {
        // IA começou a falar - mostrar faixa
        debugPrint('🌊 Transição: Esfera (Usuário) → Faixa de onda (IA falando)');
        _transitionController.forward();
        _startTextAnimation();
      } else {
        // IA parou de falar ou usuário está ativo - voltar para esfera
        debugPrint('🔵 Transição: Faixa de onda (IA) → Esfera (Usuário ativo)');
    ...
    // Forçar transição para esfera quando usuário começa a falar
    if (widget.isAISpeaking && !oldWidget.isAISpeaking) {
      debugPrint('👂 Usuário começou a falar - forçando transição para esfera');
      setState(() {
        _showingWave = false;
      });
      _transitionController.reverse();
    ...
    // Atualizar partículas baseado no som real e texto transcrito da IA
    if (_showingWave && widget.soundLevel > 0.1 && widget.transcribedText.isNotEmpty) {
      // Gerar partículas com letras quando tem transcrição
      if (_particles.length < 25) { // Limitar número de partículas
        _generateTextParticles();
      }
    }

    // Atualizar texto transcrito da IA
    if (widget.transcribedText != oldWidget.transcribedText && widget.transcribedText.isNotEmpty) {
      ...
      child: Container(
        ...
        child: Stack(
          ...
          children: [
            ...
            // FAIXA DE ONDA - Visível quando IA fala
            AnimatedBuilder(
                  ...
                  child: Container(
                    ...
                    child: Stack(
                      children: [
                        ...
                        // Texto transcrito da IA com efeito de digitação

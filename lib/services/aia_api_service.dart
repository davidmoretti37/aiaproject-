...
class AIAApiService {
  ...
  static Future<Map<String, dynamic>?> executeTask(
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        debugPrint(
          '[AIA API] 📊 Agent: ${data['agent_id']}, Success: ${data['success']}',
        );
        ...
        final normalizedData = {
          ...
          'message':
              data['message'] ?? '', // Compatibilidade com interface atual
          'agent_used': data['agent_id'] ?? 'unknown',
          'session_id': data['session_id'],
          'metadata': data['metadata'],
          'error': data['error'],
          'completed':
              data['completed'] ?? true, // Assume completo se não especificado
        };

        return normalizedData;
      } else {
        debugPrint(
          '[AIA API] ❌ Erro HTTP ${response.statusCode}: ${response.body}',
        );
        return null;
      }
    } catch (e) {
  ...
  static bool needsTaskExecution(String message) {
    ...
    if (RegExp(
      r'\b(email|e-mail|mandar|enviar|gmail|mensagem)\b',
  ...
  static String detectIntent(String message) {
    ...
    // Calendar/Agenda
    if (RegExp(
      r'\b(agendar|marcar|reunião|compromisso|evento|encontro|consulta)\b',
    ).hasMatch(lowerMessage)) {
      return 'calendar';
    }
    ...
    // Gmail/Email
    if (RegExp(
      r'\b(email|enviar|mandar|escrever|responder|gmail|mensagem)\b',
    ).hasMatch(lowerMessage)) {
      return 'gmail';
    }
    ...
    // Transporte
    if (RegExp(
      r'\b(uber|taxi|corrida|viagem|ir para|vou para)\b',
    ).hasMatch(lowerMessage)) {
      return 'transport';
    }
    ...
    // Comida
    if (RegExp(
      r'\b(ifood|comida|pedido|delivery|restaurante|lanche|pizza)\b',
    ).hasMatch(lowerMessage)) {
      return 'food';
    }
